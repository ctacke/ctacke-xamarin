...
//            var fromPageName = Analytics.GetPageName(CurrentView);
            ...
            else if(m_homeView != Application.Current.MainPage)
            {
                if (m_homeView is NavigationPage)
                {
...
//            Analytics.LogPageNavigation(fromViewName, toViewName);
        ...
        public static void ClearInstanceCache()
        {
            ...
            {
                m_viewInstanceCache.Clear();
                m_viewModelInstanceCache.Clear();
            }
        }

        private static Dictionary<Type, Page> m_viewInstanceCache = new Dictionary<Type, Page>();
        private static Dictionary<Type, IViewModel> m_viewModelInstanceCache = new Dictionary<Type, IViewModel>();
        ...
        private static object CreateViewAndViewModel(Type viewType)
        {
            ...
            lock (m_creationSyncRoot)
            {
                ...
                if (m_viewInstanceCache.ContainsKey(viewType))
                {
                    view = m_viewInstanceCache[viewType];
                }
                else
                {
                    try
                    {
                        ...
                        if (!m_viewInstanceCache.ContainsKey(viewType))
                        {
                            m_viewInstanceCache.Add(viewType, view);
                        }
                    }
                    catch (Exception ex)
                    {
            ...
            lock (m_creationSyncRoot)
            {
                ...
                {
                    viewModel = m_viewModelInstanceCache[viewModelType];
                }
                else
                {
                    try
...
//            Analytics.LogPageNavigation(null, toPage);
        ...
        internal static IViewModel GetViewModelForView(Type viewType)
        {
            ...
            lock (m_creationSyncRoot)
            {
                ...
                return m_viewModelInstanceCache[viewModelType];
            }
        }
        ...
        internal static Page GetViewForViewModel(Type viewModelType)
        {
            ...
            lock (m_index)
            {
                ...
                return m_viewInstanceCache[tuple.Key];
            }
        }
